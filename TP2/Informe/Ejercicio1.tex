...
\begin{algorithm}[H]
	...
	\Orden{$\theta$($tam(v)^2$)}
	\BlankLine		
    \For{($i = 0;i<= tam(v);i++$)\tcp*{$\theta$($tam(v)^2$))}}
{ascenso[i] $\leftarrow$ maximoAsc(ascenso,v,i) + 1\tcp*{$\theta$($i$) $\subseteq$ $\theta$($tam(v)$)}}
\end{algorithm}
...
\SetKw{Orden}{Complejidad:}
	...
	\Orden{$\theta$($tam(v)^2$)}
\BlankLine		
     \For{($i = tam(v);i \geq 0;i--$) \tcp*{$\theta$($tam(v)^2$)}}
      {descenso[i] $\leftarrow$ maximoDes(descenso,v,i) + 1\tcp*{$\theta$($tam(v) - i$) $\subseteq$ $\theta$($tam(v)$)}}
  \end{algorithm}
...
A continuación veremos gráficos que muestran el comportamiento del algoritmo utilizado
	...
	\begin{table}[h!] %ubicacion de la tabla
		\centering %centra la tabla
			\begin{tabular}{c}
				...
				\end{tabular}
				\caption{Estos gráficos muestran el tiempo de ejecución y cantidad de operaciones en secuencias de 1 hasta 100 elementos comparados con funciones cuadraticas} %titulo de la tabla
	...
	\end{table}
				...
				Este gráfico muestra la cantidad de operaciones en secuencias de 1 hasta 250 elementos comparados con una funcion cuadratica (notar que es la misma función cuadratica que en el gráfico anterior).
