...
void crearEmpresaAlAzar2(int n, ostream& os, int paso);
void crearHoraAlAzar(int& hora, int& min, int& seg, int& desde);
void crearHora(int& hora, int& min, int& seg, int desde);
bool menor(programador a, programador b);

int main(){

	generarArchivo("entrada5");
...
void generarArchivo(string nombreArchivo){
	...
	for(int i = 1; i<= cantEmpresas; i++){
		...
		crearEmpresaAlAzar2(cantProg,os,SEGXDIA/(cantProg+5));
...
void crearEmpresaAlAzar2(int n, ostream& os, int paso){
	...
	for(int i = 0; i < n; i++){
		...
		//creo horario de entrada para el programador i
		crearHora(P.hrs, P.mins, P.segs, desde);
		P.nom = i+1;
		entradas.push_back(P);
