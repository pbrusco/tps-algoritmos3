...
,G.info[*it].ejesHasta)\tcp*{\Ode{n*log(n)}}
				G.ejesUsados.insert(eje(G,*it,vertice))\tcp*{\Ode{log(n)}}}}
	}
...
\begin{algorithm}[H]
	...
	\For{(it = vEjesHasta.begin(), it $\neq$ vEjesHasta.end(), it++)\tcp*{\Ode{tam(vEjesHasta)} $\subseteq$  \Ode{n} }} {
		\If{ (wEjesHasta.count(*it) == 0)\tcp*{\Ode{log(tam(wEjesHasta))} $\subseteq$ \Ode{log(n)}}} {ejesEnCiclos.insert(*it)\tcp*{\Ode{log(tam(ejesEnCiclos))} $\subseteq$ \Ode{log(m)} $\subseteq$ \Ode{log(n^2)} $\subseteq$ \Ode{log(n)} }}
	}
...
\underline{Gráfico A} Comentar que muestra
