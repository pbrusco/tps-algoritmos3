...
void Grafo::guardarnuestro(ostream& os){
	...
	for(int i = 0; i < matrizAd.size(); i++){
		...
		for(int j = 0; j < matrizAd.size();j++){
			if (matrizAd[i][j] != 0) os << matrizAd[i][j]+1 << " ";
...
void generarGrafoAlAzar(){
	...
	int cantNodos,k,cantEjes,probabilidadEje;
	...
	cout 	<< "Ingresar:" << endl
			<< "Cantidad de nodos maxima (se van a crear grafos desde 5 nodos hasta lo que pongas aca.. multiplos de 5) ";
	cin >> cantNodosMax;
	...
	for(int n = 5; n <= cantNodosMax;n++){
		Grafo g(n);
		llenarAristas(probabilidadEje,g);
		g.guardarnuestro(os);
	}
