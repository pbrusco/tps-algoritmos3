...
void Grafo::maxClique(set<int>& res) {
	int cant_cc = detectarCC();		/* cantidad de componentes conexas */
	bool encontre;						/* flag para saber si ya se encontro una clique del tamaño buscado */
	int v, grado_v;
	heap heapGrados;
	set<int> temp_cc, vecinosFiltrados;
		...
		while (not heapGrados.empty() and heapGrados.top().first >= (int)res.size()) {
			...
			for (int j = grado_v; j >= (int)res.size() ; j--) {
				...
				if ((int)vecinosFiltrados.size() >= j) {
					...
					cliqueK(encontre, j+1, vecinosFiltrados, temp_cc);

					/* Si el tamaño de la clique que encontre es mayor que el de la actual la guardo*/
					if (res.size() < temp_cc.size()) res = temp_cc;
				}
			}
