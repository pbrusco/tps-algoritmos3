...
Por otra parte, también es observable que el algoritmo efectua los llamados recursivos en la medida que el exponente se pueda seguir dividiendo por 2. En consecuencia, se puede afirmar que el algoritmo realiza a lo sumo $q.log_2$(n) operaciones; de donde sigue que la complejidad teórica del algoritmo es \Ode{log_2(n)}.
...
Mediante cada uno de estos casos se buscó estudiar el comportamiento del algoritmo y medir su complejidad real, valiéndose para ello del conteo de la cantidad de operaciones que realiza el algoritmo para resolver el problema. No obstante, previo a la experimentación, se formularon varias hipótesis en cuanto a qué era esperable observar en cada caso en particular:
	\begin{itemize}
		...
		\item[4)]{Por último, en el caso \texttt{d}, cada llamado recursivo del algoritmo siempre ingresa al blóque de código de los exponentes impares. Por ende, realiza exactamente q operaciones $log_2$(n) veces. Luego, es presumible que la complejidad real para el caso es \texttt{d} es \titade{log_2(n)}.}
	\end{itemize}
